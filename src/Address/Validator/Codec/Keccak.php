...
final class Keccak
{
    ...
    private static $keccakf_rndc32 = [
        [0x0000, 0x0000, 0x0000, 0x0001], [0x0000, 0x0000, 0x0000, 0x8082], [0x8000, 0x0000, 0x0000, 0x0808a],
        [0x8000, 0x0000, 0x8000, 0x8000], [0x0000, 0x0000, 0x0000, 0x808b], [0x0000, 0x0000, 0x8000, 0x0001],
        [0x8000, 0x0000, 0x8000, 0x08081], [0x8000, 0x0000, 0x0000, 0x8009], [0x0000, 0x0000, 0x0000, 0x008a],
        [0x0000, 0x0000, 0x0000, 0x0088], [0x0000, 0x0000, 0x8000, 0x08009], [0x0000, 0x0000, 0x8000, 0x000a],
        [0x0000, 0x0000, 0x8000, 0x808b], [0x8000, 0x0000, 0x0000, 0x008b], [0x8000, 0x0000, 0x0000, 0x08089],
        [0x8000, 0x0000, 0x0000, 0x8003], [0x8000, 0x0000, 0x0000, 0x8002], [0x8000, 0x0000, 0x0000, 0x0080],
        [0x0000, 0x0000, 0x0000, 0x0800a], [0x8000, 0x0000, 0x8000, 0x000a], [0x8000, 0x0000, 0x8000, 0x8081],
        [0x8000, 0x0000, 0x0000, 0x8080], [0x0000, 0x0000, 0x8000, 0x00001], [0x8000, 0x0000, 0x8000, 0x8008]
    ];
    ...
    private static function keccakf64(&$st, $rounds)
    {
        ...
        for ($round = 0; $round < $rounds; $round++) {
            ...
            // Iota
            $st[0] = array(
                $st[0][0] ^ self::$keccakf_rndc64[$round][0],
                $st[0][1] ^ self::$keccakf_rndc64[$round][1]
            );
        }
    }
    ...
    private static function keccakf32(&$st, $rounds)
    {
        ...
        for ($round = 0; $round < $rounds; $round++) {
            ...
            // Iota
            $st[0] = array(
                $st[0][0] ^ self::$keccakf_rndc32[$round][0],
                $st[0][1] ^ self::$keccakf_rndc32[$round][1],
                $st[0][2] ^ self::$keccakf_rndc32[$round][2],
                $st[0][3] ^ self::$keccakf_rndc32[$round][3]
            );
        }
    }
    ...
    /**
     * @param string $in
     * @param integer $mdlen [224/256/384/512]
    ...
    public static function hash($in, $mdlen, $raw_output = false)
    {
        if (!in_array($mdlen, array(224, 256, 384, 512), true)) {
            throw new \InvalidArgumentException('Unsupported Keccak Hash output size.');
        }
    ...
    /**
     * @param string $in
     * @param integer $security_level [128/256]
     * @param integer $outlen
     * @param bool $raw_output
     * @return string
     * @throws \InvalidArgumentException
     */
    public static function shake($in, $security_level, $outlen, $raw_output = false)
    {
        if (!in_array($security_level, array(128, 256), true)) {
            throw new \InvalidArgumentException('Unsupported Keccak Shake security level.');
        }
